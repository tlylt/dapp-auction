...
const AuctionPopupContent = (props) =>{
  ...
  const handleDurationInput = (event) => {
    setVars({
      ...vars, duration: event.target.value
    })
  }
  ...
  const handleCreate = async () => {
    ...
    const dur = parseInt(vars.duration)
    let val = await auctionFactoryContract.methods.createNewAuction(vars.nftAddress, tid, bid, inc, dur).send({from: accounts[0]})
